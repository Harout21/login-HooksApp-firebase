...
function StandardClock({time}) {
    ...
    return (
        <div className='standard-clock'>
            <div>
                {clockMarks.map((mark, index) => <span key={index} className='mark'>{mark}</span>)}
            </div>
            <div>
                {Array(60).fill(1).map((tick, index) => <span key={index} className='tick'/>)}
            </div>
        ...
        <div className='readout'>
            <p className='phrases'>
                {phrases.map((phrase, index) => (
                    <span key={index} className={highlighted[index] ? 'glow' : ''}>
            {phrase}
          </span>
                ))}
            </p>
